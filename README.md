...
2.コンテナに入る
```
docker container exec -it observation-auth-app-1 bash
```
...
4.マイグレーションの実行
```
migrate -database="mysql://root:root@tcp(host.docker.internal:3306)/observation-auth?multiStatements=true" -path=docs/migration up
```
5.Swaggerのビルド
```
